...
#         print(x.shape)
#         self.projection.weight.data.abs_()
        x = self.projection(x)
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pos_embed= nn.Parameter(torch.randn(1, self.patch_embeddings.num_patches , config["hidden_size"]))
...
#             op = self.layernorm(op)
#             print(op.mean((-1,-2)).shape)
#             outp =torch.cat((outp,op.mean((-1,-2)).expand_dims(0)),dim=1)
...
#         output,input1 = self.corr1(embedding_output)
...
#         self.classifier.weight.data.abs_()
